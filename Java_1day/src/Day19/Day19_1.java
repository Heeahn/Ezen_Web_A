...
public class Day19_1 {// cs
	
	public static void main(String[] args) {// ms
		
		// 컬렉션(수집) 프레임워크(미리만들어진 클래스)
			//* 배열의 단점을 보완 사용
			// 1. List 컬렉션
				// * 인덱스 O (저장순서)
				// 1. ArrayList 클래스 		: 싱글 스레드 / 순차적 추가, 삭제
				// 2. Vector 	클래스 		: 멀티 스레드(동기화O)
				// 3. LinkedList 클래스 	: 싱글 스레드 / 중간에 삽입/삭제
			// 2. Set 컬렉션
...
//1. HashSet : 컬렉션프레임워크 클래스
		...
		// 4. Iterator : 저장된 객체를 한번씩 가져오는 반복자 리턴( 순회)
			// * set 컬렉션 에서 사용되는 이유 : 인덱스가 없기 때문에 객체를 호출하는 방법X getX
		...
		//6. set객체명.clear() : set객체내 모든 객체를 비우기[삭
